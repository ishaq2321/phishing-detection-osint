...
\begin{longtable}{
>{\raggedright\arraybackslash\ttfamily\scriptsize}p{0.27\textwidth}
>{\raggedright\arraybackslash\footnotesize}p{0.15\textwidth}
>{\raggedright\arraybackslash\footnotesize}p{0.48\textwidth}}
...
overarching \texttt{reputationScore} bounded between 0.0 and 1.0, in
which higher values denote \emph{more} suspicious findings --- a
convention the scoring engine and the API schema both follow
consistently. The summary payload additionally reports
\texttt{reputationSourcesChecked}, the number of sources that returned
usable data, so that consumers can distinguish ``checked and clean''
from ``no source ran'' (the typical case when provider API keys are
absent). This score provides the orchestrator with definitive,
community-verified intelligence that directly heavily influences the
\texttt{VerdictResult}, particularly when the machine learning model's
confidence falls into the ambiguous ``Suspicious'' tier. Because
VirusTotal's free tier permits only about four requests per minute,
successful lookups are memoised in a bounded 30-minute in-memory cache,
and a bounded concurrency semaphore caps simultaneous batch analyses so
that a full 50-item batch cannot exhaust the provider quota or the
hosting instance.
...
The system initializes nine \texttt{PhraseMatcher} pipelines and
supplements them with deterministic regular-expression detectors to
recognise a total of ten \texttt{PhishingTactic} categories. The
nine \texttt{PhraseMatcher} pipelines are: 1.
...
Detects suspicious call-to-action phrasing. 7. \texttt{emotionalMatcher}:
Detects emotional manipulation and appeal-to-greed phrases (e.g.,
``congratulations,'' ``you have been selected''). 8.
\texttt{monetaryMatcher}: Detects financial lure and money transfer
requests (e.g., ``wire transfer,'' ``processing fee,'' ``lottery
winnings''). 9. \texttt{socialProofMatcher}: Detects fake trust
signals and social proof claims (e.g., ``trusted by millions,'' ``join
millions of users''). The remaining two categories---file attachment
threats and embedded link manipulation---are detected by dedicated
regular-expression methods (\texttt{\_detectAttachments} and
\texttt{\_detectLinkManipulation}) rather than \texttt{PhraseMatcher}
pipelines, since they require pattern matching on file extensions and
URL substrings rather than whole-phrase recognition.
...
the tokenized text against a predefined list of 17 high-value
organizations: PayPal, Microsoft, Apple, Amazon, Google, Facebook,
Instagram, Netflix, LinkedIn, Dropbox, Bank of America, Wells Fargo,
Chase, IRS, FedEx, DHL, and USPS. Both the URL analyzer (via regex
patterns) and the NLP analyzer (via phrase matching) detect these
brands. Because brand mentions can occur in legitimate contexts, this
...
\subsection{Psychological Manipulation and Trust Signals}\label{psychological-manipulation-and-trust-signals}

Modern phishing campaigns increasingly leverage sophisticated
psychological manipulation tactics beyond simple urgency or threats.

\textbf{Emotional Manipulation:} The \texttt{\_detectEmotionalManipulation}
method identifies phrases designed to trigger emotional responses or appeal
to greed. These include congratulatory messages (``you have been selected,''
``congratulations''), exclusivity claims (``once in a lifetime,'' ``exclusive
offer''), and fear of missing out tactics (``don't miss out,'' ``limited
spots''). Such indicators are assigned a \texttt{severity=0.5}.

\textbf{Monetary Requests:} The \texttt{\_detectMonetaryRequests} method
flags financial lure phrases and direct money transfer requests. This
includes wire transfer requests (``wire transfer,'' ``send money''), advance
fee fraud (``processing fee,'' ``advance fee''), lottery and inheritance
scams (``claim your prize,'' ``unclaimed funds,'' ``beneficiary''), and
payment demands (``pay now,'' ``outstanding balance''). These receive a
high \texttt{severity=0.7} due to their direct financial threat.

\textbf{Attachment Malware:} The \texttt{\_detectAttachmentMalware} method
scans for references to suspicious file attachments. It detects dangerous
file extensions (\texttt{.exe}, \texttt{.scr}, \texttt{.bat}, \texttt{.vbs},
\texttt{.js}, \texttt{.zip}, \texttt{.docm}, \texttt{.xlsm}) and attachment
action phrases (``download the attachment,'' ``open the attachment,'' ``see
attached''). With \texttt{severity=0.8} for suspicious extensions and
\texttt{severity=0.65} for attachment actions, these are among the highest
severity indicators.

\textbf{Social Proof and Trust Signals:} The
\texttt{\_detectSocialProof} method identifies fake trust signals designed
to establish false credibility. These include popularity claims (``millions
of users,'' ``everyone is using''), verification assertions (``verified by,''``certified,'' ``award-winning''), and bandwagon appeals (``join millions,''``most popular''). These indicators receive a lower \texttt{severity=0.4}
but contribute to the sophistication bonus when combined with other
tactics.
...
If the final computed confidence exceeds 0.5, the \texttt{isPhishing}
boolean is asserted.
