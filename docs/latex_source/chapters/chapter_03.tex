...
Structural Features, NLP Analysis, and the budgeted OSINT collection phase}
\end{figure}
...
Crucially, this parallel execution runs under a configurable global
budget (\texttt{OSINT\_TIMEOUT}, default 25\,seconds) enforced with
\texttt{asyncio.wait}. The collection semantics were redesigned after
latency benchmarking revealed a data-loss defect in the original
\texttt{wait\_for}-based implementation: when one slow source (a flaky
DNS record type stretching past the budget) triggered the deadline, the
entire gather was cancelled and \emph{already-completed} sibling results
were discarded --- even well-established domains lost all OSINT data.
The corrected semantics keep every source that finishes within the
budget and cancel only the stragglers, and the budget itself was raised
from a hardcoded 15\,s to a configurable 25\,s: for an investigative
tool, waiting a few extra seconds is preferable to silently losing
critical data. Unresponsive external servers or rate-limited
third-party APIs therefore degrade the analysis gracefully --- the
orchestrator proceeds with whatever subset of OSINT data completed,
falling back to pure ML/NLP heuristics when nothing survives.

Two further reliability measures complete the OSINT layer. First, a
30-minute in-memory result cache shields rate-limited providers
(VirusTotal's free tier permits roughly four requests per minute) from
repeated analyses of the same domain. Second, because legacy port-43
WHOIS has poor coverage for modern gTLDs and hangs on some ccTLDs, a
WHOIS failure triggers an RDAP fallback: the Registration Data Access
Protocol, the HTTP successor to WHOIS, is queried via \texttt{rdap.org}
and yields registration dates and registrar information for TLD
families that legacy WHOIS misses entirely.
...
\subsubsection{History Export: CSV and JSON Dumps}

Operators regularly need the history off the API entirely -- feeding a
SOC analytics pipeline, archiving a week's triage, or running an
ad-hoc correlation in a spreadsheet.  Tier 4 adds two read-only bulk
export routes, \texttt{GET \hspace{0pt}/api/history/export.csv} and
\texttt{GET \hspace{0pt}/api/history/export.json}, both returning a
``Content-Disposition: attachment`` payload bounded by the store's
FIFO cap.

The CSV dump is a flat, spreadsheet-friendly projection: one row per
analysis (newest-first, matching \texttt{GET /api/history}) with the
full original content plus the flattened verdict (\texttt{isPhishing},
\texttt{threatLevel}, \texttt{confidenceScore}), the semicolon-joined
reason list, the recommendation, and the OSINT domain / reputation
score when present.  Serialisation lives in a FastAPI-free module
\texttt{backend/api/historyExport.py} so the column contract is
unit-tested in isolation; the Python \texttt{csv} module handles
quoting, so a content string containing commas, quotes, or newlines
round-trips losslessly through Excel.  The JSON dump is the lossless
complement: a plain array of the full entries with ISO-8601
timestamps, suitable for re-import or long-term archive.

One routing subtlety is worth recording: the dynamic route
\texttt{/api/history/\{entryId\}} would happily capture
``export.csv`` as an entry ID and answer 404, so the two export
routes are registered \emph{before} the parameterised one -- the
framework resolves matches in registration order.  A dedicated
regression test (\texttt{test\_exportCsv\_is\_not\_swallowed\_by\_entryId\_route})
pins the precedence.
...
The feedback sink is the write side of a deliberate two-stage improvement loop, and the read side now exists: \texttt{backend/ml/training/retrainFromFeedback.py} folds operator corrections back into the training pipeline.  The bridge reads the JSONL log, resolves each record's \texttt{historyId} against the active history store (SQLite when \texttt{PHISHGUARD\_PERSIST\_HISTORY=1}), and maps the three verdicts to labels: \texttt{false\_negative} $\rightarrow$ 1 (phishing), \texttt{false\_positive} $\rightarrow$ 0 (safe), and \texttt{correct} $\rightarrow$ the original verdict (the operator confirms the model).  For every usable record it re-extracts the 17 URL structural features offline with \texttt{extractUrlFeatures} and emits the 12 OSINT features as NaN (``unknown''): live WHOIS/DNS/reputation is deliberately not re-queried, both because it is slow and rate-limited and because infrastructure data may have changed since the original analysis -- the same convention \texttt{extractFeatures.py} already applies to unavailable OSINT data, and XGBoost handles NaN natively.  The output, \texttt{data/processed/feedback\_features.csv}, is written in exactly the same schema as \texttt{features\_raw.csv}, so it can be concatenated with the original corpus and fed straight into \texttt{prepareDataset} and \texttt{trainModel}; the downstream zero-variance column filter drops the never-populated OSINT columns exactly as it did for the original dataset, preserving the 21-feature model input.  Records that cannot contribute are skipped and counted rather than silently dropped: \texttt{historyId}s evicted from the FIFO store, non-URL content (email/text verdicts come from the NLP pipeline, not the URL classifier), unknown verdict values, and malformed lines.  The full loop -- analyse, persist, flag, re-train -- is verified end-to-end by \texttt{tests/integration/test\_feedback\_retrain\_loop.py}.
...
\section{EML Ingestion: RFC 822 / MIME Email Intake}\label{eml-ingestion}

Batch analysis and the JSON email endpoint cover analysts who already
have structured data.  A third, very real operator workflow remained
unserved: investigators forward \emph{the actual phishing message} --
as a raw ``.eml`` file exported from Outlook, Thunderbird, or a
sandbox -- and expect the system to read it.  Tier 4 adds
\texttt{POST \hspace{0pt}/api/ingest/email}, which accepts raw RFC 822
/ MIME bytes and runs them through the exact email-analysis pipeline
used by \texttt{/api/analyze/email}.

\subsection{Parser Design}

Parsing is delegated to the Python standard library's
\texttt{email.parser.BytesParser}, the reference RFC 822 / MIME
implementation -- deliberately, since it adds no dependency and is
\emph{lenient by construction}: malformed messages degrade to partial
fields instead of crashing, matching the graceful-degradation
philosophy of the rest of the API.  The extraction layer
(\texttt{backend/api/emlIngest.py}) resolves four things:

\begin{itemize}
\item
  \textbf{Headers.}  The \texttt{Subject} is decoded with
  \texttt{email.header} (RFC 2047 encoded-words such as
  \texttt{=?utf-8?Q?...?=} become readable text) and the
  \texttt{From} header is split into display name and address with
  \texttt{email.utils.parseaddr}.  \texttt{To} and \texttt{Cc}
  addresses are collected into a recipient list.
\item
  \textbf{Body.}  The MIME tree is walked depth-first: the first
  \texttt{text/plain} part wins; otherwise the first \texttt{text/html}
  part is used with tags stripped and entities decoded; otherwise all
  text parts are joined.  Quoted-printable and base64 transfer
  encodings are decoded via each part's declared charset.
\item
  \textbf{Attachments.}  Parts with a \texttt{Content-Disposition:
  attachment} header or a filename are detected and listed, but their
  payload is \emph{never} fed to the analyser -- only the extracted
  text is scored, so an embedded macro file cannot pollute the
  verdict.
\item
  \textbf{Content assembly.}  \texttt{buildEmailContent} composes
  ``\texttt{From:}\ldots\textbackslash nSubject:\ \ldots'' lines
  around the body exactly as \texttt{/api/analyze/email} does, so the
  spaCy pipeline sees identical input shape and scoring semantics.
\end{itemize}

\subsection{Guard Rails and Response}

The endpoint enforces three boundaries:

\begin{itemize}
\item
  \textbf{Size cap.}  Payloads above \texttt{settings.emlMaxBytes}
  (default 1\,MB, overridable via \texttt{EML\_MAX\_BYTES}) are
  rejected with HTTP 413 \emph{before} any parsing -- a 50\,MB
  attachment bomb never reaches the MIME tree.
\item
  \textbf{No readable body.}  A message whose extracted text contains
  no alphanumeric character at all (empty payload, header-only
  message, pure binary garbage) is rejected with 422; the NLP
  pipeline has nothing to score.
\item
  \textbf{Leniency beyond that.}  Anything carrying at least one
  readable character is analysed rather than rejected -- the
  response's \texttt{parsed} summary (subject, sender, recipients,
  attachment names, body preview, size) lets the operator see exactly
  what was read out of the file.
\end{itemize}

The response is the standard \texttt{AnalysisResponse} (verdict,
OSINT, features) extended with a \texttt{parsed} block, so the frontend
can reuse its existing result rendering -- and it does: the
\texttt{/analyze/ingest} page uploads the file, shows the parsed
fields (subject, sender, recipients, attachments, body preview) and
renders the verdict with the same banner and detail cards as every
other analysis mode.  Successful ingests are
persisted to the history store, the route is rate-limited at
\texttt{ANALYZE\_LIMIT}, and it is covered by the optional
\texttt{X-Api-Key} middleware (\texttt{/api/ingest} is in the
protected-prefix set).  A subtle integration bug found during
verification of this feature is worth recording: pydantic-settings
\texttt{< 2.2} matches environment names case-insensitively but does
\emph{not} ignore underscores, so the documented \texttt{CORS\_ORIGINS},
\texttt{VIRUSTOTAL\_API\_KEY}, and \texttt{ABUSEIPDB\_API\_KEY}
variables were silently ignored in production (the app fell back to
wildcard CORS and no reputation keys).  Every multi-word settings
field now carries an explicit \texttt{AliasChoices} alias accepting
both the camelCase field name and the UPPER\_SNAKE env var, with
regression tests pinning the mapping.

\section{Observability: Zero-Dependency Prometheus Metrics}\label{prometheus-metrics}

A detection system running unattended needs more than a health check:
the operator wants to know how much traffic it is absorbing, how slow
the heavy analysis path really is, and how the verdict mix is moving
day to day.  Tier 4 adds a Prometheus-compatible
\texttt{GET \hspace{0pt}/metrics} endpoint exposing three
signal families in the text exposition format (version 0.0.4):

\begin{itemize}
\item
  \texttt{phishguard\_http\_requests\_total\{method,path,status\}} --
  a counter of every request handled, fed by an ASGI middleware;
\item
  \texttt{phishguard\_http\_request\_duration\_seconds\{method,path\}}
  -- a latency histogram using the standard Prometheus default
  buckets (5\,ms through 10\,s), so percentile dashboards need no
  custom bucket tuning;
\item
  \texttt{phishguard\_analysis\_total\{content\_type,threat\_level\}}
  -- a counter of completed analyses, incremented in the single
  funnel every analysis path goes through
  (\texttt{AnalysisOrchestrator.analyze}), so single, batch, and
  ``.eml'' ingest analyses all count with their resolved content type
  and verdict.
\end{itemize}

\subsection{Design Decisions}

Two choices distinguish this feature from a copy-paste
\texttt{prometheus\_client} integration:

\begin{itemize}
\item
  \textbf{Zero dependency.}  The exposition format is a stable,
  simple line protocol (``\texttt{\# HELP}'', ``\texttt{\# TYPE}'',
  labelled series), so the module renders it directly instead of
  pulling in \texttt{prometheus-client}.  The deployment dependency
  tree stays unchanged, and the format is pinned by golden tests --
  including the two subtle rules that commonly break hand-rolled
  exporters: histogram suffixes (\texttt{\_bucket}/\texttt{\_sum}/
  \texttt{\_count}) come \emph{before} the label set, and the
  \texttt{le} label lives inside the same braces as the other labels.
\item
  \textbf{Self-exclusion.}  \texttt{/metrics} and \texttt{/api/health}
  are skipped by the request middleware so a scraper polling every
  15\,s and an uptime monitor never inflate their own series -- the
  same public, unthrottled posture as the health endpoint (the
  default 60/min rate limit still applies).
\item
  \textbf{Metrics can never break analysis.}  The orchestrator hook
  is fire-and-forget inside a try/except; a malformed label value or
  a bug in the exporter degrades the metrics, never the verdict.
\end{itemize}

The endpoint is operational rather than part of the documented API
(\texttt{include\_in\_schema=False}) and is not listed in OpenAPI.
Grafana or any Prometheus scraper can poll it directly, giving the
thesis's evaluation chapter a concrete, instrumented observability
story to point at.
...
\section{Continuous Integration and Containerisation}\label{ci-cd-and-docker}

After the thesis defence, the project was extended with a reproducible
engineering workflow: an automated continuous integration (CI) pipeline
and full containerisation of both application tiers. Both additions were
driven by the same principle that shaped the hardening tiers --- every
claim in this document should be verifiable by a single command.

\subsection{GitHub Actions CI Pipeline}

The pipeline is defined in \texttt{.github/workflows/ci.yml} and runs on
every push to \texttt{main} and on every pull request. It comprises three
independent jobs that mirror the local verification commands exactly:

\begin{enumerate}
  \item \textbf{Backend} --- installs \texttt{requirements.txt}, provisions
  the pinned spaCy model wheel, and executes the full pytest suite (965
  tests) on Python~3.10.
  \item \textbf{Frontend} --- performs a clean \texttt{npm ci} and runs the
  Jest suite (173 tests).
  \item \textbf{E2E} --- installs Chromium via Playwright and executes the
  31 browser tests against a development server whose API responses are
  fully mocked through \texttt{page.route} fixtures, so no live backend is
  required.
\end{enumerate}

Concurrent runs on the same reference are cancelled automatically
(concurrency groups), and both package managers use build caching keyed
on their lock files to keep total pipeline time under four minutes.

The first real CI run immediately justified its existence by surfacing two
latent defects that the long-lived local environment had masked. First,
\texttt{python -m spacy download en\_core\_web\_sm} resolves the model
version through a compatibility lookup that produced a malformed release
URL on the runner; the fix was to install a specific model wheel URL
(\texttt{en\_core\_web\_sm-3.7.1}) directly, which also makes builds
deterministic. Second, the previously pinned combination
\texttt{pytest==8.0.0} with \texttt{pytest-asyncio==0.23.3} crashes during
test collection on a fresh installation (\texttt{'Package' object has no
attribute 'obj'}), while the upgraded local versions worked. The pins were
corrected to \texttt{pytest==9.1.1} and \texttt{pytest-asyncio==1.4.0},
verified first in an isolated throwaway virtual environment before being
committed.

\subsection{Docker Images and Compose Stack}

Containerisation provides a one-command local deployment that does not
depend on any cloud platform or free-tier quota. The backend image builds
on \texttt{python:3.10-slim}: dependencies are installed in a cached
layer, the spaCy model wheel is provisioned at build time, and the trained
model artefacts are copied from \texttt{ml/models/}. Because the
application uses absolute \texttt{backend.*} imports throughout, Uvicorn
is launched from the repository root as \texttt{uvicorn
backend.main:app}.

The frontend image uses a three-stage build (\emph{deps}, \emph{builder},
\emph{runner}) on Node~20 Alpine images: dependencies are resolved once,
the production bundle is built with \texttt{NEXT\_PUBLIC\_API\_URL} as a
build argument, and only the resulting \texttt{.next} output, runtime
dependencies, and static assets enter the final image.

A \texttt{docker-compose.yml} file wires the two services together. The
API service exposes port 8000 with a container health check polling
\texttt{/api/health/live}; the web service starts on port 3000 but is
gated by \texttt{depends\_on:\ condition:\ service\_healthy}, so the UI
never accepts traffic before the API has passed its liveness probe.
Optional OSINT credentials are passed through from the host environment
and remain absent when unset, exercising the graceful-degradation paths
described earlier. The stack was verified end-to-end: after
\texttt{docker compose up -\/-build}, the API reported \texttt{healthy},
a known-phishing URL submitted through the containerised API was correctly
classified as Critical with OSINT enrichment attached, and the served
frontend returned HTTP~200.

\section{Model Drift Monitoring}\label{drift-monitoring}

A static model in a dynamic threat landscape degrades silently: phishing
infrastructure evolves (new TLD campaigns, novel URL patterns), so the
feature distribution observed in production gradually departs from the
distribution the classifier was calibrated on. PhishGuard addresses this
with a zero-dependency, zero-cost drift monitor built on the Population
Stability Index (PSI)~---~the standard distribution-shift statistic from
credit-risk modelling.

\subsection{Architecture}

The monitor consists of three cooperating pieces:

\begin{enumerate}
  \item \textbf{Feature log.} Every URL analysis funnels through the
  orchestrator (the same choke point used by the metrics middleware), which
  appends the raw 21-dimensional model-input vector to a JSONL log. The
  write is fire-and-forget: monitoring failures can never fail an analysis.
  \item \textbf{Reference snapshot (cold start).} The training dataset is
  not distributed with the repository, so no committed baseline exists.
  Instead, once \texttt{minSamples} vectors have been recorded, the oldest
  window is snapshotted into \texttt{driftBaseline.json} and serves as the
  expected distribution for all subsequent evaluations. Until then the
  report status is \texttt{cold\_start}.
  \item \textbf{Evaluation.} \texttt{GET /api/model/drift} compares the most
  recent \texttt{evaluationWindow} vectors against the snapshot. Bin edges
  are derived from expected-sample quantiles with epsilon smoothing for
  empty bins, yielding one PSI score per feature banded as stable ($<0.1$),
  moderate ($0.1$--$0.25$), or significant ($\geq 0.25$); the top-level
  \texttt{overall} field mirrors the worst feature.
\end{enumerate}

\subsection{Observability Integration}

Each evaluation is mirrored into two Prometheus gauges exposed by the
existing \texttt{/metrics} endpoint: \texttt{phishguard\_drift\_psi} (one
series per feature) and \texttt{phishguard\_drift\_max\_psi}. An operator
scraping the metrics endpoint can therefore watch model health trend over
time without touching the application. The same report is surfaced to end
users through a \emph{Model Health} card on the How-It-Works page, which
renders the overall status band and the five most-shifted features. A significant-drift signal is the
natural trigger for the retrain-from-feedback loop described in Section
\ref{persistent-operations}, closing the MLOps cycle: analyse $\rightarrow$
log $\rightarrow$ detect drift $\rightarrow$ retrain.

The implementation required no schema changes and no external services;
it reads exclusively data PhishGuard itself produces. The behaviour is
covered by 21 dedicated tests (16 unit tests over the PSI statistic,
baseline bootstrapping, torn-write tolerance, and vector mapping; 5
integration tests exercising the orchestrator hook, cold-start semantics,
and metrics exposure through the real FastAPI application).

\section{Deterministic Verdict Explanations}\label{explanation-engine}

Security tooling carries a stricter explainability requirement than most
domains: an explanation must be reproducible, auditable, and traceable to
concrete evidence. A generative language model satisfies none of these
properties --- the same input can yield different prose across calls, and
the generated text may assert relationships that do not exist in the
feature data. PhishGuard therefore explains its URL verdicts with a
template-based natural-language generation engine
(\texttt{backend/ml/explainer.py}) that contains no learned or generative
component at all.

\subsection{Signal Extraction and Ranking}

The engine evaluates the raw feature vector against a catalogue of
threshold rules, each mapping a value range onto a human-readable sentence
and one of four severity bands (critical, high, medium, low). For example,
a domain younger than 30 days produces \emph{``The domain was registered
only 6 days ago; fresh domains are a strong phishing indicator''} at
critical severity, while the absence of HTTPS yields a medium-severity
sentence. OSINT signals (fresh registration, blacklist membership,
aggregate reputation) participate in the same catalogue.

Ranking within a severity band is informed by the global signed-mean SHAP
values computed at training time and persisted alongside the model:
signals on features that historically exert stronger pull towards the
phishing class are surfaced first. The output is a structured report ---
a one-sentence plain-language summary plus ordered
\texttt{(signal,\ severity,\ detail)} items --- attached as an optional
\texttt{explanation} field on URL analysis responses. Because the field is
optional and absent for email/text analyses, the change is fully backward
compatible for existing API consumers. On the frontend the report drives a
\emph{Why this verdict?} panel on the results page: the summary sentence is
displayed prominently, and each item carries a colour-coded severity badge
together with its machine-readable signal identifier for auditability.

\subsection{Properties}

Three properties distinguish this design from an LLM-based alternative.
First, \textbf{determinism}: identical inputs always produce identical
explanations, which matters both for user trust and for regression
testing. Second, \textbf{auditability}: every sentence is bound to a named
signal identifier that a reviewer can trace back to a specific feature
value. Third, \textbf{zero cost}: the engine requires no API keys, no
network access, and adds negligible latency (~microseconds of template
evaluation). The behaviour is covered by 15 dedicated tests covering
trigger thresholds, ordering invariants, summary generation, and end-to-end
response attachment.
...
The API enforces a configurable 25-second budget on all OSINT queries using \texttt{asyncio.wait}, keeping every source that finishes and cancelling only stragglers. This prevents external service delays from blocking the response while avoiding the data loss a hard cancel-everything timeout caused. The implementation details are described in Section \ref{asynchronous-osint-orchestration}.
...
framework, comprising 1219 distinct tests (1015 backend \texttt{pytest} unit/integration tests, 173 frontend \texttt{Jest} tests, and 31 \texttt{Playwright} E2E browser tests) distributed
