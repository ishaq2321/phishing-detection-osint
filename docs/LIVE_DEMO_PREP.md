...
- `@playwright/test` (in `e2e/`) — Browser automation (13 spec files, 31 tests)
...
> "Python code quality was enforced through: (1) Pydantic's strict type validation at runtime, (2) comprehensive pytest coverage (~965 tests), and (3) manual code review. For future work, I would add `ruff` or `mypy` for static analysis."
...
# Run E2E tests (Playwright — 31 tests, 13 spec files)
...
- "Five contributions: (1) OSINT-enhanced feature engineering, (2) optimized XGBoost pipeline, (3) SHAP explainability, (4) multi-modal analysis engine, (5) production-grade full-stack application with 1141 automated tests."
...
> 1141 total: 965 backend (pytest) + 145 frontend (Jest). Plus 31 Playwright E2E browser tests.
...
| Backend tests | 965 (pytest) |
| Frontend tests | 145 (Jest) |
| E2E tests | 31 (Playwright) |
| Total tests | 1141 |
| Optuna trials | 50 |
...
**Your response:**
> "The scope of this thesis was the detection system itself — the ML pipeline, OSINT integration, and full-stack application. CI/CD is infrastructure, not research. Both Vercel and Render auto-deploy from main as basic CD. In a production environment, I would add GitHub Actions to run the 1141-test suite on every push, with deployment gated on passing tests. This is standard DevOps practice but was out of scope for the research contribution."
...
**Your response:**
> "Type safety in the backend is enforced at runtime by Pydantic v2 — every API input and output goes through strict schema validation. The 965 pytest tests cover all critical paths. For production, I would add `mypy` for static type checking and `ruff` for linting. The decision to omit them was a time-management trade-off — the thesis deadline prioritized functional completeness over tooling polish."
...
## 12.12 Limited E2E Test Coverage (31 tests)

**The gap:** 31 Playwright E2E tests vs. 965 backend unit tests. The E2E coverage is thin.

**Your response:**
> "The testing strategy prioritizes depth where it matters most — the ML pipeline and OSINT integration have 965 rigorous unit and integration tests. The 31 E2E tests cover all critical user journeys: URL analysis, email analysis, batch analysis, EML ingestion, history, settings, theme, navigation, responsive design, and error handling. For a thesis prototype, this coverage is comprehensive. In production, I would expand E2E coverage for edge cases."
...
- 1141 total tests (965 backend + 145 frontend + 31 E2E)
