...
Chapter \ref{scoring-engine-and-evaluation}) yielded a profound observation regarding the integration of
...
This defensive posture is reinforced at the HTTP boundary itself. The
production deployment ships hardened response headers (HSTS,
Strict-Transport-Security, Content-Security-Policy with
\texttt{frame-ancestors\ 'none'}, X-Frame-Options, and a strict
Referrer-Policy), per-key rate limits that throttle abusive clients
before they can exhaust OSINT quota, \texttt{X-Api-Key} authentication
on the heavy analysis routes with SHA-256-digested, constant-time
key comparison, structured JSON logs with a per-request
\texttt{X-Request-ID} for incident correlation, and a deep
\texttt{/api/health} endpoint that probes DNS, the XGBoost model
load state, and the orchestrator import chain in parallel and
returns a stratified \texttt{healthy} / \texttt{degraded} /
\texttt{unhealthy} verdict. Render's liveness probe
(\texttt{healthCheckPath}) is additionally routed to a side-band cheap
\texttt{/api/health/live} endpoint so the per-second scan never burns
the readiness budget. Together these hardenings transform the
detection pipeline into a service that can survive operator
oversight, opportunistic attackers, and silent dependency drift --
the three failure modes most likely to occur in the months after
the system is deployed.

Operator-side persistence is the second dimension of production
maturity.  The \texttt{backend/api/historyStore.py} module exposes
an opt-in SQLite-backed implementation of \texttt{HistoryStore} so
analysis history survives an application restart; the existing
in-memory deque remains the default to preserve test determinism.
The slowapi limiter accepts the standard \texttt{limits}-library
URI through \texttt{PHISHGUARD\_RATE\_LIMIT\_URI}, so a
multi-instance Render deployment can swap the in-process bucket for
a Redis-backed one without a code change.  Finally,
\texttt{backend/api/feedback.py} materialises operator verdicts
against past analyses into an append-only JSONL log
(\texttt{./data/feedback.jsonl}); the companion listing endpoint
\texttt{GET /api/feedback} returns the parsed record list so an
analyst dashboard can sanity-check recent submissions.

Bulk operator workflows are the third dimension.  Section
\ref{batch-analysis} adds the \texttt{POST /api/analyze/batch}
endpoint, which closes the frontend's ``Batch -- up to 50 URLs''
feature by exposing a single-round-trip path that reuses the same
\texttt{AnalysisOrchestrator::analyze()} pipeline (one round of
OSINT, ML, and NLP per item) but dispatches the items concurrently
via \texttt{asyncio.gather}.  A 50-item batch completes in
approximately the time of a single item rather than 50 times as
long, while the \texttt{@limiter.limit(ANALYZE\_LIMIT)} decorator
keeps the operator's bucket accounting identical to the
single-content routes -- one round trip per minute, not fifty.
Per-item partial failures are surfaced in-band so a single
malformed URL never blocks an analyst's triage session.
...
The SHAP feature importance analysis (Figure \ref{fig:shap_plot}) highlighted
...
\section{Post-Development Hardening: A Reliability Case
Study}\label{post-development-hardening}

Following the initial delivery, a systematic review and
production-verification pass surfaced and repaired a series of defects
that collectively form a instructive case study in distributed-systems
hygiene. Three are worth recording.

First, a latency benchmark of the live OSINT collectors exposed a
silent data-loss mechanism: the original collection phase cancelled
\emph{all three} parallel lookups when its global deadline expired,
discarding results that had already completed. A single flaky DNS
record type (resolvers frequently drop TXT or MX queries) could thus
strip a well-established domain of every OSINT signal. The remedy --- a
keep-what-finished collection semantic, a configurable 25-second
budget, per-record-type deadlines, a quota-protecting result cache, and
an RDAP fallback for the TLD families legacy WHOIS cannot serve ---
restored complete OSINT coverage for every reachable domain measured.

Second, a semantic-direction defect was caught by cross-checking the
API schema against its consumer: the aggregated reputation score
encodes \emph{suspicion} (higher is worse), but the client rendered it
as \emph{trust} (higher is better), meaning a domain flagged by
VirusTotal would have been displayed in green. The repair introduced an
explicit \texttt{reputationSourcesChecked} counter so that ``verified
clean'' and ``no source ran'' are distinguishable at a glance --- an
honesty property the interface previously lacked.

Third, the deployment pipeline itself was found to be the source of
production drift on two separate occasions: a path-matching bug in the
frontend build filter silently skipped every push (git pathspecs are
working-directory-relative, and the filter was written for the wrong
directory), and the backend service ran with auto-deployment disabled,
requiring a manual dashboard action per release. Both were corrected
with path-aware, fail-open build filters verified against every real
push range, and the incident is a concrete demonstration that
\emph{deployment configuration is code} and demands the same
adversarial review as application logic.

The general lesson aligns with the thesis's central theme: reliability
is not a feature to be declared but a property to be measured,
verified, and re-verified against the running system.
