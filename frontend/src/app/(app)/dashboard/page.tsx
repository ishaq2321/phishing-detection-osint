                    ...
                    <div>
                      ...
                      <p className="mt-1 text-2xl font-bold">{stats.total}</p>
                       <p className="text-xs text-muted-foreground mt-1">
                         {stats.total > 0 ? "Most recent analyses" : "No data yet"}
                       </p>
                    </div>
                      ...
                      {entries.map((entry, index) => {
                        ...
                        return (
                          <div
                            key={entry.id}
                            className={cn(
                              "grid grid-cols-[auto_1fr] items-center gap-4 px-4 py-3 sm:grid-cols-[auto_1fr_128px_44px_84px]",
                              !isLast && "border-b"
                            )}
                          >
                            ...
                            {/* Score Bar — fixed 128px column */}
                            <div className="hidden sm:flex items-center gap-2 col-span-2 mt-2 sm:col-span-1 sm:mt-0 w-full sm:w-[128px]">
                            ...
                            {/* Percentage — fixed 44px column, right-aligned */}
                            <span className="hidden sm:block text-xs font-medium tabular-nums text-right w-[44px]">
                              {Math.round(entry.score * 100)}%
                            </span>

                            {/* Threat badge — fixed 84px column, centered text */}
                            <Badge
                              variant="secondary"
                              className={cn(
                                "hidden sm:inline-flex justify-center w-[84px] shrink-0 text-xs",
                                meta.colorClass,
                              )}
                            >
                              {meta.label}
